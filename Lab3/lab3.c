...
void SysTick_Handler (void) {                    // SysTick Interrupt Handler
  ...
  prev_sensor_states = sensor_states;
	system_update_state(&system, logged_in_user, msTicks);
...
void Device_Initialization (void)  {             // Configure & Initialize MCU
  ...
  system.sensor_states = &sensor_states;  // pass sensor states by reference
...
void EXTI2_IRQHandler (void) {
  if (system.state == ALARMED) printf ("^beep^ \n"); // alarm indicator
